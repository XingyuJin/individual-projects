...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while nearest_place != hive and current_distance <= self.max_range:
            has_bees = nearest_place.bees
            if has_bees and self.min_range <= current_distance:
                return random_or_none(has_bees)
            else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        bee_list = list(current_place.bees)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def inspire_ants_behind(self):
        """Return the Ants behind the Queen."""
        place_behind = self.place.exit

        while place_behind:
            ant = place_behind.ant
            if ant:
                if not ant.queen_inspired:
                    ant.damage *= 2
                    ant.queen_inspired = True
                if ant.container and isinstance(ant.ant, Ant) and not ant.ant.queen_inspired:
                    ant.ant.damage *= 2
                    ant.ant.queen_inspired = True
...
# Status Effects #
##################
def next_movement(self, colony):
    if  self.effected_turn:
        if self.effected_turn[0][1] <= 0:
            del self.effected_turn[0]
            next_movement(self, colony)
        else:
            self.action = types.MethodType(self.effected_turn[0][0](self.action), self)
    else:
        self.action = lambda x: type(self).action(self, x)


def STUN_ACTION(self,colony):
    if self.effected_turn:
        self.effected_turn[0][1] -= 1
    next_movement(self,colony)
...
def SLOW_ACTION(self, colony):
    if self.effected_turn:
        self.effected_turn[0][1] -= 1
        if colony.time % 2 == 0:
            self.effected_turn = [self.effected_turn[0]] + [[pair[0], pair[1]-1] if pair[0] == make_slow else [pair[0], pair[1]] for pair in self.effected_turn[1:]]
            next_movement(self, colony)
            return type(self).action(self, colony)
    next_movement(self,colony)
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem E
    if not hasattr(bee, "effected_turn"):
        bee.effected_turn = []
    bee.effected_turn.insert(0, [effect, duration])
    bee.action = types.MethodType(bee.effected_turn[0][0](bee.action), bee)
    "*** YOUR CODE HERE ***"
    # END Problem EC
