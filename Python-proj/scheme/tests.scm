...
(equal? '(1 . ((2 . 3))) (cons 1 (cons (cons 2 3) nil)))
; expect True
...
(define x 1)
(+ x
   (begin (print x) (define x (* 3 (+ 2 x))) (+ 2 x))
   10)
; expect 1 ; 22

(define (add-a-lot x y z) (+ x (+ 3 y) (* 4 z)))
; expect add-a-lot

add-a-lot
; expect (lambda (x y z) (+ x (+ 3 y) (* 4 z)))
...
(define (f n)
  ...
  (j 0 0))
j
; expect (mu (c k) (if (< c n) (j (+ c 2) (- k 1)) k))

f
; expect (lambda (n) (define c n) (j 0 0))
