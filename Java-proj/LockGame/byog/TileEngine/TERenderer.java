...
public class TERenderer implements Serializable {
    ...
    public void renderFrame(TETile[][] world, Player player) {
        ...
        drawInfo(player, numXTiles, numYTiles);
        StdDraw.show();
    }

    public void drawInfo(Player player, int w, int h) {
        int midWidth = w / 2;
        int midHeight = h / 2;
        ...
        StdDraw.text(midWidth, height - 1, "Go and Explore!");
        StdDraw.textLeft(1, height - 2, "Life: " + Integer.toString(player.getLife()));
        StdDraw.text(midWidth, height - 2, "Current Position:" + player.getInfo());
        StdDraw.textRight(width - 1, height - 2, "Key Left:"
                + Integer.toString(player.getKeyNum()));
