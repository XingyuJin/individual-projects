...
/**
 *  The {@code Trie} class represents an symbol table of key-value
...
public class Trie<Value> {
    private int n;              // size
    private Node<Value> root;   // roots of Trie
    ...
    private void collect(Node<Value> x, StringBuilder prefix, LinkedList<String> list) {
        if (x == null) {
            return;
        }
    ...
    private void collect(Node<Value> x, StringBuilder prefix,
                         int i, String pattern, LinkedList<String> list) {
        if (x == null) {
            return;
        }
        char c = pattern.charAt(i);
        if (c == '.' || c < x.c) {
            collect(x.left, prefix, i, pattern, list);
        }
        if (c == '.' || c == x.c) {
        ...
        if (c == '.' || c > x.c) {
            collect(x.right, prefix, i, pattern, list);
        }
    }
}
